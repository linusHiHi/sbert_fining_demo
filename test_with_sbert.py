...
def test(input_texts_, candidate_tests_, accept_rate_, input_class_tag_=3):
    ...
    for i in range(len(input_texts_)):
        ...
        print(f"original: {input_texts_[i]}")
        for idx in top_indices:
            if similarities[i][idx] > accept_rate_:
                if candidate_tests_["class"][idx] == input_class_tag_(i):
                    print(f"Text: {candidate_tests_["sentence"][idx]}, Similarity Score: {similarities[i][idx]}, class: {input_class_tag_(i)}")
                else:
...
test_df = test(input_texts, candidate_texts, accept_rate, input_class_tag_=input_class_tag)
